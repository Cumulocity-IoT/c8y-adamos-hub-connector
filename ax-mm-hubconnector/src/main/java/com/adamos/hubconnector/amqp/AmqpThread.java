...
public class AmqpThread implements Runnable, HealthIndicator {
	...
	public boolean connect() {
		...
		try {
			...
			connectionFactory = new CachingConnectionFactory(new URI(globalSettings
					.getAdamosAmqpEndpoint() + "/" + username));
			...
			while (connection == null || !connection.isOpen()) {
				try {
					...
					LOGGER.error("AMQP-connection to \"" + globalSettings.getAdamosAmqpEndpoint() + "\" failed...");
					...
					try {
						...
						// maximum slowdown should be capped with 5 minute (60 * 50 = 300 sec = 5 min)
						// intervals
