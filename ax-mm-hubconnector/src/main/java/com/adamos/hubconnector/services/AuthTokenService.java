...
public class AuthTokenService {
	...
	private OAuth2Token getNewTokenViaRefreshToken(String refreshToken, OAuth2Credentials credentials) {
        ...
        RequestEntity<LinkedMultiValueMap<String, String>> requestEntity = new RequestEntity<LinkedMultiValueMap<String, String>>(body, headers, HttpMethod.POST, URI.create(appProperties.getOAuthEndpoint()));
    ...
    public OAuth2Token getToken() {
		...
		if (this.getCurrentToken() == null || this.getCurrentToken().getAccessToken() == null) {
			...
			if (DateTime.now().isAfter(token.getExpiryDate())) {
				try {
					// get a new token with the help of the refresh-token we got with the last session
