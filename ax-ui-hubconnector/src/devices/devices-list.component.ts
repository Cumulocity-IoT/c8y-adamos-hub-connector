...
export class DevicesListComponent {
    ...
    constructor(private inventory: InventoryService, private hubService: AdamosHubService, private bsModalService: BsModalService, private modalService: ModalService, private router: Router) {
    ...
    async loadDevices() {
        ...
        switch (this.activeFilter) {
            case "DISCONNECTED":
                ...
                this.filter['query'] = "$filter=has('c8y_IsDevice') and not has('adamos_hub_isDevice')" + orderBy;
                break;
            case "CONNECTED":
                this.title = "Connected Devices";
                this.filter['query'] = "$filter=has('adamos_hub_isDevice')" + orderBy;
                break;
            default:
                this.title = "All Devices";
                this.filter['query'] = "$filter=has('c8y_IsDevice') or has('adamos_hub_isDevice')" + orderBy;
                break;
        }
    ...
    async onImportClick() {
        ...
        forkJoin([this.hubService.getDisconnectedEquipment$(), this.hubService.getGlobalSettings$()]).subscribe(results => {
            this.equipmentList = results[0];
            this.selectedIsDevice = results[1].defaultSyncConfiguration.hubToAdamos.c8yIsDevice;
    ...
    onImportDeviceClick() {
        if (this.selectedHubDeviceUuid != null) {
            ...
            this.hubService.importHubDevice$(this.selectedHubDeviceUuid, this.selectedIsDevice).subscribe(device => {
                this.router.navigate(['/devices', device.id.value]);
                this.isDialogDataImporting$.next(false);
                this.modalRef.hide();
            });
        } else {
            this.modalService.confirm(_("Info"), _("Please select a device to import?"), 'info', { ok: _("OK")});
        }
    }
