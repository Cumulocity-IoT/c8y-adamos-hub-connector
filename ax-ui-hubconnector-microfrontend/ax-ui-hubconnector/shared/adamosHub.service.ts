...
export class AdamosHubService {
    ...
    constructor(private http: HttpClient, private auth: BasicAuth, private cookieAuth: CookieAuth, private inventory: InventoryService, private alertService: AlertService) { 
        ...
        const basicAuth = this.auth.getCometdHandshake();
        const authn = get(<any>basicAuth?.ext, 'com.cumulocity.authn');
        if (authn?.token) {
            headers = headers.set('Authorization', 'Basic ' + authn.token);
        } else {
            const authn = get(<any>this.cookieAuth.getCometdHandshake().ext, 'com.cumulocity.authn');
            headers = headers.set('X-XSRF-TOKEN', authn.xsrfToken);
        }
    ...
    addAsset (id: number): Observable<IManagedObject> {
        ...
        return this.http.post<IManagedObject>(url, "", this.httpOptions).pipe(
            tap((asset: IManagedObject) => console.log(`added asset w/ id=${asset.id}`)),
    ...
    async getMappings() {
        ...
        const { data } = await this.inventory.list(filterMappings);
        return data;
    }
    ...
    static createEventRule(): IEventRule {
        ...
        set(response.eventTrigger, '@type', 'HubEventTrigger');
